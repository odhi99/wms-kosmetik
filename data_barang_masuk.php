...
<body>
            ...
            <div class="container-fluid">
                <div class="row page-titles">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item"><a href="javascript:void(0)">Data barang masuk</a></li>
                    </ol>
                </div>
                <div class="row">
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                ...
                                <button type="button" class="btn btn-outline-primary" id="tambah" data-id="<?= $dataNama['id_admin_221013']; ?>" data-bs-toggle="modal" data-bs-target="#tambahdata">+ Tambah Data</button>
                            </div>
                            <div class="card-body">
                                <div class="table-responsive">
                                    <table id="example" class="display" style="min-width: 845px">
                                        ...
                                        <tbody>
                                            <?php
                                            require 'controller/koneksi.php';
                                            $result = mysqli_query($koneksi, "SELECT * FROM data_barang_masuk_221013 join data_barang_221013 join data_distributor_221013 join data_admin_221013 on data_barang_masuk_221013.id_barang_221013 = data_barang_221013.id_barang_221013 and data_distributor_221013.id_dist_221013 = data_barang_masuk_221013.id_dist_221013 and data_admin_221013.id_admin_221013 = data_barang_masuk_221013.id_admin_221013");
                                            $i = 1;
                                            while ($row = mysqli_fetch_array($result)) {
                                            ?>
                                                <tr>
                                                    <td><?= $i; ?></td>
                                                    <td><?= $row['nama_admin_221013']; ?></td>
                                                    <td><?= $row['nama_dist_221013']; ?></td>
                                                    <td><?= $row['nama_barang_221013']; ?></td>
                                                    <td><?= $row['jumlah_masuk_221013']; ?></td>
                                                    <td><?= $row['tgl_masuk_221013']; ?></td>
                                ...
                                <div class="row">
                                    <div class="col-md-12 mt-3">
                                        <div class="form-group">
                                            <input type="text" value="<?= $dataNama['id_admin_221013']; ?>" id="id_admin" disabled hidden>
                                            <label for="text-input2" class="form-label">Pilih Distributor</label>
                                            <select id="tdist" class="form-control">
                                                <option selected>--pilih--</option>
                                                <?php
                                                $result = mysqli_query($koneksi, "select * from data_distributor_221013");
                                                while ($row = mysqli_fetch_array($result)) {
                                                ?>
                                                    <option value="<?= $row['id_dist_221013']; ?>"><?= $row['nama_dist_221013']; ?></option>
                                                <?php
                                                }
                                                ?>
                                            </select>
                                    ...
                                    <div class="col-md-12 mt-3">
                                        <div class="form-group">
                                            <label for="text-input2" class="form-label">Pilih Barang</label>
                                            <select id="tbarang" class="form-control">
                                                <option selected>--pilih--</option>
                                                <?php
                                                $result = mysqli_query($koneksi, "select * from data_barang_221013");
                                                while ($row = mysqli_fetch_array($result)) {
                                                ?>
                                                    <option value="<?= $row['id_barang_221013']; ?>"><?= $row['nama_barang_221013']; ?></option>
                                                <?php
                                                }
                                                ?>
                                            </select>
                                    ...
                                    <div class="col-md-12 mt-3">
                                        <div class="form-group" id="thasil">

                                        </div>
                                    </div>
                                    <div class="col-md-12 mt-3">
                                        <div class="form-group">
                                            <label for="text-input2" class="form-label">Stok Masuk</label>
    ...
    ***********************************-->
        ...
        <script type="text/javascript">
            $('#tbarang').change(function() {
                var barang = $(this).val();
                $.ajax({
                    type: 'POST',
                    url: 'controller/user/barang_masuk.php',
                    data: 'id_barang=' + barang,
                    success: function(response) {
                        $('#thasil').html(response);
                    }
                });
            });
        </script>
        ...
        <script type="text/javascript">
            ...
            function tambah() {
                ...
                let id_dist = $('#tdist').val();
                let barang = $('#tbarang').val();
                let stok = $('#tstok').val();
                let pilih = 1;
                if (barang == "" || stok == "" || id_dist == "") {
                    Swal.fire('', 'field tidak boleh kosong', 'error');
                } else {
                    let fd = new FormData();
                    fd.append('id_admin', id_admin);
                    fd.append('id_dist', id_dist);
                    fd.append('barang', barang);
                    fd.append('stok', stok);
                    fd.append('pilih', pilih);
                    $.ajax({
                        url: "controller/user/controller_masuk.php",
                        type: "POST",
                        data: fd,
                        contentType: false,
                        processData: false,
                        success: function(response) {
                            ...
                            if (response === "sukses") {
                                swal.fire({
                                    ...
                                    if (isConfirm) {
                                        window.location.replace("data_barang_masuk.php");
                                    }
                                });
            ...
            $(document).on("click", "#edits", function() {
                ...
                // $('#utype').val(type);
                $('#ustok').val(stok);
                document.getElementById('utype').innerHTML = '<option>' + nmtype + '</option>';
